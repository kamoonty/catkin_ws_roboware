...
{
  // ROS objects
  ros::init(argc, argv, "vl_trajectory_diagonal");
ros::NodeHandle nh;
...
while (ros::ok())
  ...
  else if (t>=12&&t<22) //Diagonal
   {final_cmd_vel.linear.y=0.2;
   final_cmd_vel.linear.x=0.2;
    ROS_INFO("Sideways +Y");}       
  else if (t>=22&&t<32) //delay for 3secs
    {final_cmd_vel.linear.x=0.2;
    final_cmd_vel.linear.y=0;
    ROS_INFO("Go straight");} 
   else if (t>=32&&t<42) ////Sideway
   {final_cmd_vel.linear.y=-0.2;
   final_cmd_vel.linear.x=0.2;
    ROS_INFO("Sideways -Y");} 
    else if (t>=42&&t<52) //delay for 3secs
    {final_cmd_vel.linear.x=0.2;
    final_cmd_vel.linear.y=0;
    ROS_INFO("Go straight");} 
  else if(t>=52)
