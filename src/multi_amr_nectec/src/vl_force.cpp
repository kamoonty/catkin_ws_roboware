...
{  get_vl.x=roundf(vl_pos.x*100000)/100000; // round to 5 decimal place
   get_vl.y=roundf(vl_pos.y*100000)/100000;
...
{ for (int i = 0; i<index; i++)
    { robot_pos[i].x=roundf((msg->point_robot[i].x)*100000)/100000; // round to 5 decimal place
      robot_pos[i].y=roundf((msg->point_robot[i].y)*100000)/100000;
...
int main(int argc, char** argv) 
{
 ...
 float hold_x=dist_from_vl_x; // use for triangular shape formation
...
{ 
   for (int j = 0; j < team_size; j++)
      {          
                int check_x=1;
                int check_y=1;
           // ROS_INFO("robot pos=[%.3f,%.3f] VL pos =[%.3f,%.3f]",robot_pos[j].x,robot_pos[j].y,get_vl.x,get_vl.y);
              ...
              //check_y 
               ...
               //uncomment if the shape is not triangle      
                if (j==2) //top position of triangle 
              {dist_from_vl_y=dist_from_vl_y_triangle; 
               dist_from_vl_x=0;
              }  
              else
              ...
              dist_from_vl_x=hold_x;}
               ...
               ROS_INFO("Dist from VL of AMR %d=[%.3f,%.3f]",j,Dist_vl[j].x,Dist_vl[j].y);
               ROS_INFO("Absolute distance of AMR %d from VL =%f",j,abs_dist);
                ...
                //ROS_INFO("Virtual Force of robot %d[%.3f,%.3f]",j,Force_vl[j].x,Force_vl[j].y);           
                 ...
                 ROS_INFO("Cmd_vel robot %d x=%.3f y=%.3f",j, send_fvl[j].linear.x,send_fvl[j].linear.y);  
                 ROS_INFO("-----------------------");
                }
